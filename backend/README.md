...
- Optimistic locking (Phase 2): `bookedCount` counter guarded by `@Version`, `VERSION_CONFLICT` 409 handling, verified under real concurrent load with a standalone Java test script
...
- Cancelling a `CONFIRMED` booking automatically promotes the oldest waitlisted booking on that resource to `CONFIRMED`, in the same transaction as the cancellation. Cancelling a `WAITLISTED` booking simply removes it from the queue — it does not free a slot and triggers no promotion.
